...
/* =====================================
	Starting off the program (default)
======================================== */

let savedToDoList = localStorage.getItem('toDoState');
if (savedToDoList === null) {
	document.getElementById("ul-task-list").innerHTML = `
		<li class="li-task">Add some new todo's<span class="close">×</span></li>
		<li class="li-task">Cross off the ones you're done with<span class="close">×</span></li>
...
function printUpdateNotification() {
	...
	document.getElementById('update-notification').innerHTML =
		`
		<p class="update-notification-text">Open me for new themes!</p>
		<button class="dismiss-notification" id="dismiss-notification">✕</button>
		`;
	...
	document.getElementById("update-notification").classList.add("update-notification-shown");
}

let lastOpened = localStorage.getItem('cd-last-opened');

let lastVersion = localStorage.getItem('cd-last-version');

if (lastVersion == null || lastVersion != currentVersion) {
	printUpdateNotification();
} else if (lastOpened != null) {
	lastOpened = new Date(lastOpened);
	let lastOpenedMonth = lastOpened.getMonth();
	let currentMonth = new Date().getMonth();
	if (lastOpenedMonth != currentMonth &&
		(currentMonth == MAR || currentMonth == JUN || currentMonth == SEP || currentMonth == DEC)
	) {
		printUpdateNotification();
	}
}
...
function dismissNotification() {
	document.getElementById('update-notification').innerHTML = '';
	document.getElementById("update-notification").classList.remove("update-notification-shown");
	localStorage.setItem('cd-last-opened', new Date());
	localStorage.setItem('cd-last-version', currentVersion);
}